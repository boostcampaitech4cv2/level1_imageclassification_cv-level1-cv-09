...
def kfold_train(data_dir, model_dir, args):
    ...
    num_classes = dataset.num_classes  # Depends on your type
    ...
    for fold, (train_idx, val_idx) in enumerate(skfold.split(X = dataset.image_paths , y = dataset.labels)):
        os.makedirs(f"{save_dir}/fold{fold}", exist_ok = True)
        ...
        for epoch in range(args.epochs):
            ...
            for idx, train_batch in enumerate(train_loader):
                ...
                if (idx + 1) % args.log_interval == 0:
                    ...
                    current_lr = get_lr(optimizer)
                    print(
                        f"Epoch[{epoch}/{args.epochs}](Fold{fold+1}/5)]({idx + 1}/{len(train_loader)}) || "
                        f"training loss {train_loss:4.4} || training accuracy {train_acc:4.2%} || lr {current_lr}"
                    )
                    logger.add_scalar("Train/loss", train_loss, epoch * len(train_loader) + idx)
                    logger.add_scalar("Train/accuracy", train_acc, epoch * len(train_loader) + idx)
        ...
        #Get-average loss
            ...
            epoch_avg_loss = sum(global_val_losses) / len(global_val_losses)
            ...
            with open(f"{save_dir}/fold{fold}/record.txt", "a") as f:
                f.write(f"EPOCH {epoch+1} Total_val_acc : {epoch_avg_acc}, Total_loss: {epoch_avg_loss}, Best_val_acc: {best_val_acc}, Best_val_loss: {best_val_loss}")

            logger.add_scalar("Val/loss", epoch_avg_loss, epoch)
            logger.add_scalar("Val/accuracy", epoch_avg_acc, epoch)
            logger.add_figure("results", figure, epoch)
            print()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--augmentation', type=str, default='CustomAugmentation', help='data augmentation type (default: Custom_BaseAugmentation)')
    parser.add_argument("--resize", nargs="+", type=list, default=[128, 96], help='resize size for image when training')
    parser.add_argument('--batch_size', type=int, default=64, help='input batch size for training (default: 64)')
    parser.add_argument('--valid_batch_size', type=int, default=1000, help='input batch size for validing (default: 1000)')
    parser.add_argument('--model', type=str, default='BaseModel', help='model type (default: BaseModel)')
    parser.add_argument('--optimizer', type=str, default='Adam', help='optimizer type (default: Adam)') #SGD
